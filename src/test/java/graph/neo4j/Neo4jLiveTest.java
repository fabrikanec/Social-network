...
public class Neo4jLiveTest {
    ...
    public void setUp() {
        ...
        graphDb = graphDbFactory.newEmbeddedDatabase(new File("data/graphdb/friends"));
    }
    ...
    public void testUserCar() {
        ...
        Node owner = graphDb.createNode(Label.label("User"));
        owner.setProperty("firstName", "baeldung");
        owner.setProperty("lastName", "baeldung");

        owner.createRelationshipTo(car, RelationshipType.withName("owner"));

        Result result = graphDb.execute("MATCH (c:Car) <-[owner]- (p:User) " +
                "WHERE c.make = 'tesla'" +
